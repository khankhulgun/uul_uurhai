...
class MGLhuuli extends StatefulWidget {
  @override
  _MGLhuuliState createState() => _MGLhuuliState();

}
class _MGLhuuliState extends State<MGLhuuli> {
  final GlobalKey<ScaffoldState> _scaffoldKey = new GlobalKey<ScaffoldState>();

  List<MUlsHuuli$Query$Paginate$AaMUlsHuuli> huuls = [];
  bool loading = true;
  int currentPage = 1;
  int lastPage = 0;
  int total = 0;
  ...
  void getData(int page) async {
    ...
    final response = await client.execute(MUlsHuuliQuery(variables: MUlsHuuliArguments(page: page, size: 10)));
    setState(() {
      huuls = response.data.paginate.aaMUlsHuuli;
      currentPage = page;
  ...
  Widget buildTripCard(BuildContext context, int index) {
    ...
    return Container(
      ...
      child: ListTile(
        title: Text(law.huuli, style: TextStyle(fontSize: 14, color: Colors.black, fontWeight: FontWeight.w600)),
        subtitle: new Column(
